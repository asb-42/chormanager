...
from ..history.service import HistoryService, CreateSingerCommand, UpdateSingerCommand, DeleteSingerCommand
...
class SingerDialog(QDialog):
    ...
    def _setup_ui(self):
        """Set up the UI."""
        self.setWindowTitle("Sänger hinzufügen" if not self.singer else "Sänger bearbeiten")
...
class MainWindow(QMainWindow):
    ...
    def _create_menu_bar(self):
        ...
        export_availability_action = QAction("Verfügbarkeit exportieren (JSON)...", self)
    ...
    def _export_libreoffice(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["libreoffice", "--headless", "--convert-to", output_format, "--outdir", out_dir, temp_csv],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def _export_singers_json(self):
        ...
        default_path = self.db_path.replace(".db", "_singers.json") if self.db_path else "singers.json"
        filename, _ = QFileDialog.getSaveFileName(
    ...
    def _export_events_json(self):
        ...
        default_path = self.db_path.replace(".db", "_termine.json") if self.db_path else "termine.json"
        filename, _ = QFileDialog.getSaveFileName(
    ...
    def _export_availability_json(self):
        ...
        default_path = self.db_path.replace(".db", "_verfuegbarkeit.json") if self.db_path else "verfuegbarkeit.json"
        filename, _ = QFileDialog.getSaveFileName(
        ...
        if filename:
            ...
            try:
                ...
                self.statusBar().showMessage(f"Verfügbarkeit exportiert nach {filename}")
                QMessageBox.information(self, "Export", f"Exportiert nach:\n{filename}")
            except Exception as e:
                QMessageBox.warning(self, "Fehler", f"Export fehlgeschlagen:\n{str(e)}")
    ...
    def _export_singers_csv(self):
        ...
        default_path = self.db_path.replace(".db", "_singers.csv") if self.db_path else "singers.csv"
        filename, _ = QFileDialog.getSaveFileName(
    ...
    def _new_projekt(self):
        """Create new project."""
        self.tabs.setCurrentIndex(0)
        QMessageBox.information(self, "Neues Projekt", "Bitte im Tab 'Projekte' auf 'Hinzufügen' klicken.")
    ...
    def _save_projekt(self):
        ...
        if project:
            QMessageBox.information(
                self, "Speichern",
                f"Projekt '{project.name}' ist bereits gespeichert."
            )
        else:
            QMessageBox.warning(
                self, "Speichern",
    ...
    def _open_choraufstellung_file(self, filepath: str = None):
        ...
        import os
        import sqlite3
        from PyQt6.QtWidgets import QMessageBox, QInputDialog, QDialog, QVBoxLayout, QLabel, QPushButton
        
        choraufstellung_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/choraufstellung"
        ...
        db_path = self.db_path or os.path.expanduser("~/.local/share/chormanager/chor.db")
        ...
        if event and os.path.exists(db_path):
            try:
                ...
                debug_info += f"Sänger mit Zusage für {event.date[:10]} ({len(rows)}):\n"
                for row in rows:
                    debug_info += f"  - {row['short_name'] or row['full_name']} ({row['voice_group']}) [{row['status']}]\n"
                
                cursor.execute("SELECT date, event_type FROM events ORDER BY date DESC LIMIT 5")
                debug_info += f"\nLetzte Events: {cursor.fetchall()}"
                
                cursor.execute("SELECT e.id, e.date, e.name, COUNT(a.id) as cnt FROM events e LEFT JOIN availability a ON a.event_id = e.id GROUP BY e.id ORDER BY e.date DESC LIMIT 10")
    ...
    def _get_data_dir(self):
        ...
        return Path(config.get("app", {}).get("data_dir", "~/.local/share/chormanager")).expanduser()

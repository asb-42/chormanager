...
from PyQt6.QtWidgets import (
    ...
    QRadioButton, QButtonGroup, QLabel, QScrollArea, QWidget, QGroupBox
)
...
class EventDialog(QDialog):
    ...
    def _setup_ui(self, db=None):
        ...
        self.setWindowTitle("Termin hinzufügen" if not self.event else "Termin bearbeiten")
...
class EventAvailabilityDialog(QDialog):
    ...
    def _load_availability(self):
        ...
        print(f"DEBUG dialog: besetzung_ids = {self.besetzung_ids}")
        singers = self.singer_repo.get_active()
        print(f"DEBUG dialog: total active singers = {len(singers)}")
        ...
        yes_count = sum(1 for s in singers if self.avail_repo.get_by_ids(s.id, self.event.id) and self.avail_repo.get_by_ids(s.id, self.event.id).status == "yes")
        no_count = sum(1 for s in singers if self.avail_repo.get_by_ids(s.id, self.event.id) and self.avail_repo.get_by_ids(s.id, self.event.id).status == "no")
        none_count = sum(1 for s in singers if not self.avail_repo.get_by_ids(s.id, self.event.id) or self.avail_repo.get_by_ids(s.id, self.event.id).status == "none")
        
        total_yes = yes_count
        total_conditional = sum(1 for s in singers if self.avail_repo.get_by_ids(s.id, self.event.id) and self.avail_repo.get_by_ids(s.id, self.event.id).status == "conditional")
        ...
        if voice_group_yes or voice_group_conditional:
            ...
            all_vgs = set(list(voice_group_yes.keys()) + list(voice_group_conditional.keys()))
    ...
    def _export_pdf(self):
        ...
        elements.append(Paragraph(f"Datum: {self.event.date[:10]} | Typ: {self.event.event_type}", styles['Normal']))
        elements.append(Spacer(1, 0.3 * cm))
        
        status_symbols = {"yes": "✓", "no": "✗", "none": "○", "conditional": "✓?", "unknown": "?", "maybe": "~"}
        
        table_data = [["Name", "Stimmgruppe", "Status"]]
        for singer in singers:
            avail = self.avail_repo.get_by_ids(singer.id, self.event.id)
            status = status_symbols.get(avail.status if avail else "", "-")
            table_data.append([singer.short_name or singer.full_name or "", singer.voice_group or "", status])
    ...
    def _export_libreoffice(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
            ...
            for singer in singers:
                avail = self.avail_repo.get_by_ids(singer.id, self.event.id)
                status = status_symbols.get(avail.status if avail else "", "-")
                writer.writerow([singer.short_name or singer.full_name or "", singer.voice_group or "", status])
        ...
        try:
            ...
            result = subprocess.run(
                ["libreoffice", "--headless", "--convert-to", "xls", "--outdir", out_dir, temp_csv],
                capture_output=True,
                text=True,
                timeout=30
            )
...
class ConfigDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        reset_backup_btn.clicked.connect(lambda: self.backup_dir_input.setText("./data/backups"))
        ...
        reset_log_btn.clicked.connect(lambda: self.log_file_input.setText("./data/logs/chormanager.log"))
        ...
        reset_chor_btn.clicked.connect(lambda: self.choraufstellung_path_input.setText("/media/data/coding/choraufstellung"))
    ...
    def _load_content(self):
        ...
        if row:
            ...
            if updated_at:
                ...
                self.last_modified_label.setText(f"Zuletzt bearbeitet: {dt.strftime('%d.%m.%Y %H:%M')}")
